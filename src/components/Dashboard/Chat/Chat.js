...
/**
 * Chat component
 * - Maintains full message history and sends it to the backend on each submit
 * - Displays assistant responses with summary, citations, and like/unlike controls
 * - Provides simple weighting controls for recency and sample size
 */
const Chat = () => {
  ...
  const simulateAssistantResponse = (conversation) => {
    // Very simple placeholder that echoes last user message with mock summary & sources
  ...
  // Seed a welcome message on first render
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Placeholder: simulate network latency and response
      await new Promise((resolve) => setTimeout(resolve, 800));
      const data = simulateAssistantResponse(
  ...
  const handleFeedback = async (messageId, liked) => {
    ...
    try {
      // Placeholder: mimic a successful feedback submission
      await new Promise((resolve) => setTimeout(resolve, 200));
      // eslint-disable-next-line no-console
      console.log('Feedback submitted (placeholder):', { messageId, liked });
    } catch (err) {

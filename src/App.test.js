  ...
  const getToken = async () => {
    try {
      const resp = await instance.acquireTokenSilent({
        ...loginRequest,
        account: account || (await instance.getAllAccounts())[0],
      });
      setToken(resp.accessToken);
    } catch (silentErr) {
      // Fallback to popup if silent fails (e.g., first run/consent)
      try {
  ...
  return (
    <main style={styles.page}>
      <section style={styles.card} aria-label="MSAL smoke test">
        ...
        <p style={styles.sub}>
          Use this screen only to verify Microsoft sign-in. Your styled login lives elsewhere.
        </p>

        {!isSignedIn ? (
          <button onClick={signIn} style={styles.primaryBtn} aria-label="Sign in with Microsoft (test)">
          ...
          <>
            <div style={styles.infoBox} aria-live="polite">
              ...
              <p style={styles.value}>
                {account?.name || "(no name)"} · {account?.username}
              </p>
            </div>

            <div style={styles.actions}>
              <button onClick={getToken} style={styles.secondaryBtn} aria-label="Acquire token">
                Get Access Token
              </button>
              <button onClick={signOut} style={styles.linkBtn} aria-label="Sign out">
                Sign out
              </button>
            </div>
...
const styles = {
  page: {
    ...
    placeItems: "center",
    background:
      "linear-gradient(135deg, #8e0d26 0%, #9f102d 55%, #c11a3a 100%)",
    padding: 24,
  },
  card: {
  ...
  primaryBtn: {
    ...
    height: 50,
    borderRadius: 12,
    border: "2px solid #c11a3a",
    background: "#fff",
    color: "#111827",
    fontWeight: 700,
    cursor: "pointer",
    boxShadow: "0 8px 18px rgba(16,24,40,.10)",
  },
  infoBox: {
    marginTop: 6,
    padding: "10px 12px",
